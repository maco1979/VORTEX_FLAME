#!/usr/bin/env python3
"""Cezanne 7B Retrain — Stage2: Math + Logic + Debug
From Base model with accumulated data (S1+S2), r=8
...
def main():
    ...
    print(f"  {SOUL_NAME} {STAGE}: Math + Logic + Debug", flush=True)
    print(f"  Base: {BASE_MODEL}", flush=True)
    print(f"  Data: S1(math) + S2(logic/debug) accumulated", flush=True)
    ...
    all_samples = []
    for fname in ["cezanne_s1_math_v2.json", "cezanne_s2_logic_debug_v2.json"]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        log_data = {
            ...
            "base_model": BASE_MODEL, "data_sources": ["s1_math", "s2_logic_debug"], "attempt": attempt,
