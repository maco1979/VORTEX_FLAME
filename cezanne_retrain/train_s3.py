#!/usr/bin/env python3
"""Cezanne 7B Retrain — Stage3: Math + Logic + CS Depth
From Base model with accumulated data (S1+S2+S3), r=8
No merging, no nested LoRA — single LoRA on Base
"""
...
REGRESSION_QUESTIONS = [
    ...
    {"q": "What is the derivative of x^3 * sin(x)?", "kw": ["product rule", "3x^2", "cos(x)"]},
]
...
def main():
    ...
    print(f"  {SOUL_NAME} {STAGE}: Math + Logic + CS Depth", flush=True)
    print(f"  Base: {BASE_MODEL}", flush=True)
    print(f"  Data: S1(math) + S2(logic/debug) + S3(CS) accumulated", flush=True)
    ...
    all_samples = []
    for fname in ["cezanne_s1_math_v2.json", "cezanne_s2_logic_debug_v2.json", "cezanne_s3_cs_depth_v2.json"]:
    ...
    print(f"  Total accumulated: {len(all_samples)} items", flush=True)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        print("  Skipping regression test (OOM risk in WSL). Run separately.", flush=True)
        ...
        log_data = {
            ...
            "base_model": BASE_MODEL, "data_sources": ["s1_math", "s2_logic_debug", "s3_cs_depth"], "attempt": attempt,
            "regression": "skipped_OOM_risk",
