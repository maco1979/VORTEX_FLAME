#!/usr/bin/env python3
"""Cezanne 7B Stage3b — Anti-forget + Deep + FormalProof + KeywordPatch, continues from Stage3a"""
...
S3A_LORA = r"D:\VORTEX_FLAME\soul_lora_v2\cezanne\stage3a\final"
...
def main():
    ...
    print(f"  {SOUL_NAME} Stage3b: Anti-Forget + Proof + Keywords", flush=True)
    print(f"  From Stage3a LoRA: {S3A_LORA}", flush=True)
    print(f"  Epochs: {EPOCHS}, LR: {LR}, SeqLen: {MAX_SEQ_LENGTH}", flush=True)
    print(f"{'='*60}", flush=True)

    print("  Loading data...", flush=True)
    s3b_path = os.path.join(DATA_DIR, "cezanne_stage3b_supplement_v1.json")
