#!/usr/bin/env python3
"""Cezanne_PRO 8B Stage3a - CodeAlpaca fusion, continues from Stage2 LoRA"""
...
S2_LORA = r"D:\VORTEX_FLAME\soul_lora_v2\cezanne_8b\stage2\final"
...
def main():
    ...
    out_dir = os.path.join(LORA_DIR, "stage3a")

    print(f"{'='*60}", flush=True)
    print(f"  {SOUL_NAME} Stage3a: CodeAlpaca Fusion", flush=True)
    ...
    s3a_path = os.path.join(DATA_DIR, "cezanne_pro_8b_stage3a_code_v1.json")
    with open(s3a_path, "r", encoding="utf-8") as f:
        samples = json.load(f)
    ...
    print("  Stage2 LoRA loaded (frozen, continuing on top)", flush=True)
    ...
    log_path = os.path.join(LOG_DIR, "stage3a_result.json")
