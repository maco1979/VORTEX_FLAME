#!/usr/bin/env python3
"""Cezanne_PRO 8B Stage3b - anti-forget + deep + supplements, continues from Stage3a"""
...
S3A_LORA = r"D:\VORTEX_FLAME\soul_lora_v2\cezanne_8b\stage3a\final"
...
def main():
    ...
    out_dir = os.path.join(LORA_DIR, "stage3b")

    print(f"{'='*60}", flush=True)
    print(f"  {SOUL_NAME} Stage3b: Anti-Forget + Deep + Supplements", flush=True)
    print(f"  From Stage3a LoRA: {S3A_LORA}", flush=True)
    print(f"  Epochs: {EPOCHS}, LR: {LR}, SeqLen: {MAX_SEQ_LENGTH}", flush=True)
    print(f"{'='*60}", flush=True)

    print("  Loading data...", flush=True)
    s3b_path = os.path.join(DATA_DIR, "cezanne_pro_8b_stage3b_supplement_v1.json")
    with open(s3b_path, "r", encoding="utf-8") as f:
        samples = json.load(f)
    samples = [s for s in samples if isinstance(s, dict) and len(s.get("output", "")) >= 50]
    print(f"  Loaded: {len(samples)} samples", flush=True)

    print("  Loading model + Stage3a LoRA...", flush=True)
    ...
    model = PeftModel.from_pretrained(model, S3A_LORA)
    print("  Stage3a LoRA loaded (frozen, continuing on top)", flush=True)
    ...
    lora_config = LoraConfig(
        ...
        lora_dropout=0.05, target_modules=lang_target_modules, bias="none",
    )
    ...
    log_data = {
        ...
        "base_model": BASE_MODEL, "continued_from": S3A_LORA,
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
    }
    log_path = os.path.join(LOG_DIR, "stage3b_result.json")
