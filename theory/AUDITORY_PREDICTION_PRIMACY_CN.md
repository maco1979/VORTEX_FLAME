...
这一原则从感官模态进一步推广到认知模态。**不懂编程的普通人，与先天失明者在结构上是同构的：** 我们缺少一种关键模态——代码语法、底层张量操作——但依然通过可感知的模态（自然语言、直觉、因果推理）构建完整的认知世界模型。我们知道自己想构建什么、为什么重要、应该如何运转，即使我们无法亲手写出实现。UPP天花板直接延伸：

$$\text{AI效用} \leq \min\big(\text{人类语义理解深度}, \text{AI实现能力}\big)$$

跨模态对齐让我们可以将底层实现交给AI，正如视障用户将视觉感知交给听觉线索。我们不需要学会代码；AI不需要学会自然语言——双方只需在各自获得独立能力后，在共享语义上对齐。**所有智能，都是用自己拥有的模态独立构建世界模型。对齐是桥梁，不是地基。**
