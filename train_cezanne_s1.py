#!/usr/bin/env python3
"""
Cezanne Stage1 - Code/Logic Foundation Training
Base: E:\models\Mistral-7B-Instruct-v0.1-Cezanne (clean copy)
Data: cezanne_55gb_v3.json + cezanne_hq_10k.json
"""
...
FAULT_LOG = open(r"D:\VORTEX_FLAME\hermes_logs\cezanne\fault.log", "w")
...
def load_cezanne_data(max_samples):
    ...
    hq_path = os.path.join(DATA_DIR, "cezanne_hq_10k.json")
    supp_path = os.path.join(DATA_DIR, "cezanne_desktop_supplement.json")
    k4_path = os.path.join(DATA_DIR, "cezanne_4k.json")
...
def run_exam(model, tokenizer):
    ...
    for i, q in enumerate(EXAM_Q):
        ...
        print(f"  Q{i+1}/10 [{status}] {answer[:80]}...")
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  Cezanne Stage1: Code/Logic Foundation")
    ...
    data_path = os.path.join(DATA_DIR, "cezanne_stage1_data.json")
    ...
    log_data = {
        "soul": "cezanne", "stage": "stage1", "cn_name": "Cezanne",
